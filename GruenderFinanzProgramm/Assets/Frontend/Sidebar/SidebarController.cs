...
public class SidebarController : MonoBehaviour
{
    ...
    {
        if (selected == _activeItem) return;  // schon aktiv → nichts tun
        // Alten aktiven Zustand entfernen
        _activeItem?.RemoveFromClassList("nav-item--active");
        // Neuen Zustand setzen
        selected.AddToClassList("nav-item--active");
        _activeItem = selected;
        // Seite wechseln
        string page = selected.name.Replace("nav-item-", "");
        Debug.Log($"Navigiere zu: {page}");
        // TODO: Hier deine Seiten-Logik aufrufen, z.B.:
        // NavigateToPage(page);
    }
}