...
public class EinstellungenController : MonoBehaviour
{
    ...
    {
        DataBase db = GetCurrentUserDatabase();
        ...
        // Da jede Nutzer-DB separat ist, reicht hier UserId 1.
        return db.getOrCreateSettingsForUser(1);
    }

    private DataBase GetCurrentUserDatabase()
    {
        DataBase db = UserDatabaseAccess.getCurrentUserDatabase();

        if (db == null)
        {
            Debug.LogWarning("[Einstellungen] Keine aktive Nutzer-Datenbank gefunden.");
            return null;
        }
    ...
    private void LoadLocalSettings()
    {
        ...
        if (settings == null)
        {
            Debug.LogWarning("[Einstellungen] Settings konnten nicht aus der Datenbank geladen werden. Fallback auf PlayerPrefs.");
            ...
            bool fallbackDark = PlayerPrefs.GetInt(PREF_DARK_MODE, 1) == 1;
            UpdateModeButtons(fallbackDark);
        ...
        if (_inputRechnrPraefix != null) _inputRechnrPraefix.value = settings.rechnungsNrPräfix ?? "RE-";
        if (_inputStartnummer != null) _inputStartnummer.value = settings.startNr ?? "1";
        if (_inputZahlungsziel != null) _inputZahlungsziel.value = settings.zahlungsziel ?? "14";
        if (_inputZahlungshinweis != null) _inputZahlungshinweis.value = settings.zahlungshinweis ?? "Bitte überweisen Sie den Betrag auf ...";

        if (_dropdownWaehrung != null) _dropdownWaehrung.index = settings.waehrung;
        if (_dropdownDatumsformat != null) _dropdownDatumsformat.index = settings.dtmFormat;
        if (_toggleUstRechnung != null) _toggleUstRechnung.value = settings.ustRechnung;
        if (_toggleAutoNummer != null) _toggleAutoNummer.value = settings.autoNummer;

        // Bank
        if (_inputKontoinhaber != null) _inputKontoinhaber.value = settings.kontoInhaber ?? "";
        if (_inputIban != null) _inputIban.value = settings.iban ?? "";
        if (_inputBic != null) _inputBic.value = settings.bic ?? "";
        if (_inputKreditinstitut != null) _inputKreditinstitut.value = settings.kreditinstitut ?? "";
        if (_toggleIbanRechnung != null) _toggleIbanRechnung.value = settings.ibanRechnung;

        // Rechnungseinstellungen
        if (_toggleLogo != null) _toggleLogo.value = settings.logo;
        if (_toggleSeitenzahl != null) _toggleSeitenzahl.value = settings.seitenzahl;
        if (_toggleExportpfad != null) _toggleExportpfad.value = settings.exportpfad;

        _selectedSteuersatz = settings.steuersatz <= 0 ? 19 : settings.steuersatz;
        _customSteuersatz = false;

        if (_inputSteuerCustom != null)
        {
            _inputSteuerCustom.value = "0";
        }

        UpdateSteuersatzButtons();

        // Dark Mode bleibt erstmal global/lokal, weil das nicht in Pontus' Liste als User-DB-Pflicht steht.
        bool isDark = PlayerPrefs.GetInt(PREF_DARK_MODE, 1) == 1;
        UpdateModeButtons(isDark);

        if (_toggleBegleiter != null) _toggleBegleiter.value = settings.Begleiter;

        // Bezahlweise
        if (_inputAgb != null) _inputAgb.value = settings.agb ?? "";
        if (_inputDisclaimer != null) _inputDisclaimer.value = settings.disclaimer ?? "";
        if (_inputBarzahlung != null) _inputBarzahlung.value = settings.barzahlung ?? "";
        if (_inputUeberweisung != null) _inputUeberweisung.value = settings.ueberweisung ?? "";

        Debug.Log("[Einstellungen] Settings aus Nutzer-Datenbank geladen.");
    }
    ...
    private void SaveLocalSettings()
    {
        ...
        if (db != null)
        {
            Settings settings = db.getOrCreateSettingsForUser(1);

            if (settings != null)
            {
                // Rechnung / Nummerierung
                if (_inputRechnrPraefix != null)
                    settings.rechnungsNrPräfix = _inputRechnrPraefix.value;

                if (_inputStartnummer != null)
                    settings.startNr = _inputStartnummer.value;

                if (_inputZahlungsziel != null)
                    settings.zahlungsziel = _inputZahlungsziel.value;

                if (_inputZahlungshinweis != null)
                    settings.zahlungshinweis = _inputZahlungshinweis.value;

                if (_dropdownWaehrung != null)
                    settings.waehrung = _dropdownWaehrung.index;

                if (_dropdownDatumsformat != null)
                    settings.dtmFormat = _dropdownDatumsformat.index;

                if (_toggleUstRechnung != null)
                    settings.ustRechnung = _toggleUstRechnung.value;

                if (_toggleAutoNummer != null)
                    settings.autoNummer = _toggleAutoNummer.value;

                // Bankdaten
                if (_inputKontoinhaber != null)
                    settings.kontoInhaber = _inputKontoinhaber.value;

                if (_inputIban != null)
                    settings.iban = _inputIban.value;

                if (_inputBic != null)
                    settings.bic = _inputBic.value;

                if (_inputKreditinstitut != null)
                    settings.kreditinstitut = _inputKreditinstitut.value;

                if (_toggleIbanRechnung != null)
                    settings.ibanRechnung = _toggleIbanRechnung.value;

                // Rechnungseinstellungen
                if (_toggleLogo != null)
                    settings.logo = _toggleLogo.value;

                if (_toggleSeitenzahl != null)
                    settings.seitenzahl = _toggleSeitenzahl.value;

                if (_toggleExportpfad != null)
                    settings.exportpfad = _toggleExportpfad.value;

                settings.steuersatz = _selectedSteuersatz;

                if (_toggleBegleiter != null)
                    settings.Begleiter = _toggleBegleiter.value;

                // Texte / Hinweise
                if (_inputAgb != null)
                    settings.agb = _inputAgb.value;

                if (_inputDisclaimer != null)
                    settings.disclaimer = _inputDisclaimer.value;

                if (_inputBarzahlung != null)
                    settings.barzahlung = _inputBarzahlung.value;

                if (_inputUeberweisung != null)
                    settings.ueberweisung = _inputUeberweisung.value;

                db.updateSettingsForUser(settings);

                Debug.Log("[Einstellungen] Settings in Nutzer-Datenbank gespeichert.");
            }
        }
        else
        {
            Debug.LogWarning("[Einstellungen] DB nicht verfügbar. Settings werden nur lokal gespeichert.");
        }
