...
public class FinanceDashboardBinder : MonoBehaviour
{
    ...
    private IEnumerator InitUI()
    {
        ...
        if (cGruend != null)
        {
            ...
            cGruend.Add(MakeRow("Summe Gründungskosten", 0f, TextColor, true));
        }
    ...
    private void RegistriereHelpTooltips(VisualElement root)
    {
        HelpTooltip.Registriere(root, "btn-help-seitentitel",
            "Der Finanzierungsscreen zeigt eine detaillierte Aufschlüsselung " +
            "aller Einnahmen, Ausgaben und Kosten deines Unternehmens.");

        HelpTooltip.Registriere(root, "btn-help-ertragsquellen",
            "Zeigt alle Einnahmequellen kategorisiert, den Gesamtumsatz, " +
            "die Gesamtkosten und den daraus resultierenden Rohgewinn.");

        HelpTooltip.Registriere(root, "btn-help-direkte-kosten",
            "Direkte Kosten entstehen unmittelbar durch die Leistungserbringung: " +
            "Gehälter, Honorare und sonstige direkte Ausgaben.");

        HelpTooltip.Registriere(root, "btn-help-betriebsausgaben",
            "Betriebsausgaben sind laufende Kosten des Unternehmens: " +
            "Marketing, Reisekosten, Steuern, Tilgung und Privatentnahmen.");

        HelpTooltip.Registriere(root, "btn-help-investition",
            "Geplante Investitionen in Sachanlagen wie Ausstattung und Geräte. " +
            "Diese Werte werden manuell gepflegt.");

        HelpTooltip.Registriere(root, "btn-help-gruenderkosten",
            "Einmalige Kosten die bei der Unternehmensgründung anfallen: " +
            "Corporate Design, Homepage und Grundausstattung.");

        HelpTooltip.Registriere(root, "btn-help-kapitalbedarf",
            "Gesamter Kapitalbedarf aus Investitionen, Gründerkosten " +
            "und Liquiditätsreserve — gegenübergestellt mit verfügbarem Kapital.");
    }
    ...
    private VisualElement MakeRow(string name, float wert, Color wertFarbe,
        bool fett = false, string suffix = " €")
    {
        var row = new VisualElement();
        row.style.flexDirection     = FlexDirection.Row;
        row.style.justifyContent    = Justify.SpaceBetween;
